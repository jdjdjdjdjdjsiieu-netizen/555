...
# Резервный провайдер: OpenAI-совместимый API
...
def generate_referral_message(contact_info: str, chat_history: str) -> str:
    ...
    стратегии резервирования (GigaChat -> OpenAI).
    """
    ...
    try:
        # Попытка 1: Использовать основной провайдер (GigaChat)
        ...
        print("Переключение на резервный провайдер (OpenAI)...")
        
        try:
            # Попытка 2: Использовать резервный провайдер (OpenAI)
            message = rag_chain_secondary.invoke(payload)
            print("Резервный провайдер (OpenAI) успешно сгенерировал сообщение.")
            return message
        except Exception as e_secondary:
            print(f"Ошибка при работе с резервным провайдером: {e_secondary}")
            return "Ошибка: Оба LLM-провайдера недоступны. Не удалось сгенерировать сообщение."
